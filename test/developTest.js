...
const testClean = require('./test-clean.js');
...
describe('develop', () => {
  ...
  it('updates tsconfig.json with proper paths', async () => {
    ...
    expect(config.compilerOptions.paths['@test/package2'][0]).to.be.equal(
      'develop/repo2'
    );
    expect(config.compilerOptions.paths.repo3[0]).to.be.equal(
      'develop/repo3/lib/core'
    );
    expect(config.compilerOptions.paths['@test/forms'][0]).to.be.equal(
      'develop/repo4/lib/forms'
    );
  });
  ...
  it('preserves existing paths outside src/develop', async () => {
    ...
    const raw = fs.readFileSync('./test/tsconfig-1.json');
    const config = JSON.parse(raw);
    expect(config.compilerOptions.paths.something[0]).to.be.equal(
      'dist/something'
    );
  });
